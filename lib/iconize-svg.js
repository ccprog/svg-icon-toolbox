...
module.exports = function (sourceFn, root, opt, callback) {
    ...
    spawn('inkscape -S '+ sourceFn, listViewBox, false, (err) => {
        ...
        async.each(list, (id, next) => {
            if (!dimensions[id]) {
                return utils.handleErr(`object ${id} not found in loaded source.`, 'SVG', next);
            }
            ...
            var transform = utils.computeTransform(
                ...
                $copy.attr('height'),
                $copy.attr('viewBox'),
                $copy.attr('preserveAspectRatio')
            );
            if (transform.length) {
                var preTransform = $copy.attr('transform');
                if (preTransform) {
                    transform.push(preTransform);
                }
                $copy.attr('transform', transform.join(' '));
            }
            $copy.attr('preserveAspectRatio', null);
            //crop
            ...
            async.waterfall([
                async.apply(utils.normalize, targetFn + '.svg'),
                (targetFn, next) => {
                    exportFn = targetFn;
