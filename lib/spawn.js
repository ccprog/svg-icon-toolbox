...
module.exports = function (cmd, shell, cmdCallback) {
    ...
    which(args.shift(), (err, exec) => {
        ...
        child.stderr.on('data', (err) => {
            ...
            // pass noncritical warnings from inkscape
            if (/WARNING/i.test(msg)) return;
            // using the oldest stdin command is the best guess
            var oldest = interesting.findIndex((obj) => obj !== undefined);
            if (shell && oldest >= 0) {
                utils.raiseErr(
                    msg,
                    interesting[oldest].command,
                    interesting[oldest].callback
                );
            } else {
                utils.raiseErr(msg, cmd, cmdCallback);
            }
        });
